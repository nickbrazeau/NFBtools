...
MIDPrimerFinder <- function(design_fasta = NULL,
  ...
  hits <- lapply(list(fw, rv), function(x) {
    vmatchPattern(as.character(x), tg) # coercion here is fine since dnastrinset of length 1 ... becomes a named character vector
  })
  ...
  if( all(sapply(c(MID2MIDmatchesAllowed, MIDlength, MIDhomopolymercutoff, MID2targetmismatchesAllowed, MIDnum), class) != "numeric") ){
    stop("Your MID specifications all must be of class numeric")
  }

  if( !all( c(MID2MIDmatchesAllowed, MIDlength, MIDhomopolymercutoff, MID2targetmismatchesAllowed, MIDnum) %in% sapply(c(MID2MIDmatchesAllowed, MIDlength, MIDhomopolymercutoff, MID2targetmismatchesAllowed, MIDnum), floor)) ){
  ...
  while( length(finalMIDs) < n ){
    ...
    propMIDs <- replicate(n, MIDmaker(MIDlength, MIDhomopolymercutoff)) # init
    ...
    while( any( TRUE %in% c(Biostrings::reverseComplement(Biostrings::DNAStringSet(propMIDs)) %in% Biostrings::DNAStringSet(propMIDs)) ) | # catch any reverse complements -- repropose
        ...
        if(revcount > 0){
          propMIDs[ Biostrings::reverseComplement(Biostrings::DNAStringSet(propMIDs)) %in% Biostrings::DNAStringSet(propMIDs) ] <- replicate(revcount, MIDmaker(MIDlength, MIDhomopolymercutoff))
        }
        ...
        if(length(ot_vect) > 0){
        propMIDs[ot_vect] <- replicate(length(ot_vect), MIDmaker(MIDlength, MIDhomopolymercutoff))
        }
    }
    ...
    if(int%%10 == 0){cat(paste0("Done with proposal iteration", int, "\n ------- \n"))}
